...
DiscoRenderer::DiscoRenderer(Window & parent) : OGLRenderer(parent)
{
	rotation = 0.0f;
	...
	cubeMap = SOIL_load_OGL_cubemap(TEXTUREDIR"ss_rt.tga",
		TEXTUREDIR"ss_lf.tga",
		TEXTUREDIR"ss_up.tga",
		TEXTUREDIR"ss_dn.tga",
		TEXTUREDIR"ss_bk.tga",
		TEXTUREDIR"ss_ft.tga",
		SOIL_LOAD_RGB,
...
void DiscoRenderer::GenerateScreenTexture(GLuint & into, bool depth) {
...
void DiscoRenderer::UpdateScene(float msec) {
	camera->UpdateCamera(msec);
	viewMatrix = camera->BuildViewMatrix();
	rotation = msec * 0.01f;
}

void DiscoRenderer::RenderScene() {
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
void DiscoRenderer::FillBuffers() {
	glBindFramebuffer(GL_FRAMEBUFFER, bufferFBO);
	glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);
...
void DiscoRenderer::DrawPointLights() {
...
void DiscoRenderer::CombineBuffers() {
