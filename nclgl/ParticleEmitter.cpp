...
ParticleEmitter::ParticleEmitter(string texturePath) : ParticleEmitter()
{
	texture = SOIL_load_OGL_texture(texturePath.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_COMPRESS_TO_DXT);
}
...
void ParticleEmitter::Update(float msec) {
	...
	//Skips first update to avoid weird particle spawning issues
	if (firstPass) {
		firstPass = false;
	}
	else {
...
void ParticleEmitter::SetTexture(string path)
{
	texture = SOIL_load_OGL_texture(path.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_COMPRESS_TO_DXT);
}
